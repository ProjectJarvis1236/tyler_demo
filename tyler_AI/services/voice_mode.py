...
class VoiceService:
    def __init__(self, wakeword: str, process_func=None, default_chat_id="voice", default_model="qwen/qwen3-235b-a22b-thinking-2507"):
        ...
        self.gigaam_model = onnx_asr.load_model("gigaam-v3-e2e-rnnt") # сихронная операция, у нас ассихронный Fast Api, может стоит запускать отдельным потоком, сокращается время ожидания ответа на несколько секунд
    ...
    async def record_and_recognize(self):
        ...
        while True:
            ...
            if len(audio_buffer) > 10 * self.SAMPLE_RATE * 2:
                print("Слишком длинная команда, обрезаю")
                break

        if not speech_started or len(audio_buffer) < self.FRAME_SIZE * 5:
            print("Команда не распознана (речь не обнаружена)")
            return
        ...
        if result:
            ...
            if text:
                ...
                if self.process_func:
                    ...
                    print("[VoiceService] process_func не задана")
            else:
